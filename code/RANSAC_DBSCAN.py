...
def dbscan_refinement(pts, eps, min_samples):
    """
    ...
    id_counter = np.max(segment_ids) + 1
    ...
    for segment_id in segment_ids:
        ...
        labels = dbscan.fit_predict(segment_points)
        ...
        for cluster_label in np.unique(labels):
            ...
            pts[cluster_indices, 3] = id_counter  # Update segment IDs
            id_counter += 1
...
def detect(lazfile, params, viz=False):
    ...
    # DBSCAN refinement
    dbscan_eps = params.get("dbscan_eps")
    dbscan_min_samples = params.get("dbscan_min_samples")
